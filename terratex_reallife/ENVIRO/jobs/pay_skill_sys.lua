...
local jobSetupTable = {
    ...
    [17] = {
        --Meeresreiniger
        "meeres",
        { 5500, 25000, 75000, 150000, 500000 },
        { [0] = 0.025, 0.05, 0.1, 0.25, 0.5, 0.75 },
        { 1, 1 },
        1, -- Randfaktor
        true, -- Randfaktor vor Teilung
...
function giveJobGehalt(thePlayer, jobID, auftraege, entfernung, definedMoney)
    if (jobSetupTable[jobID] and isElement(thePlayer)) then
        ...
        if (definedMoney ~= nil) then
            ...
            local geldProSkill;
            if (type(jobSetupTable[jobID][3][skill]) == "table") then
                geldProSkill = jobSetupTable[jobID][3][skill][1]
                gesamtGehalt = entfernung * jobSetupTable[jobID][3][skill][2]
            else
