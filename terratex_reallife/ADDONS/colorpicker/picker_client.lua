...
function colorPicker.create(id, start, title)
  ...
  cp.gui.svmap = guiCreateStaticImage(16, 32, 256, 256, "ADDONS/colorpicker/blank.png", false, cp.gui.window)
  cp.gui.hbar = guiCreateStaticImage(288, 32, 32, 256, "ADDONS/colorpicker/blank.png", false, cp.gui.window)
  cp.gui.blank = guiCreateStaticImage(336, 32, 64, 64, "ADDONS/colorpicker/blank.png", false, cp.gui.window)
...
function colorPicker:render()
  ...
  dxDrawImage(x+16, y+32, 256, 256, "ADDONS/colorpicker/sv.png", 0, 0, 0, self.color.white, self.gui.focus)
  dxDrawImage(x+288, y+32, 32, 256, "ADDONS/colorpicker/h.png", 0, 0, 0, self.color.white, self.gui.focus)
  dxDrawImageSection(x+8+math.floor(256*self.color.s), y+24+(256-math.floor(256*self.color.v)), 16, 16, 0, 0, 16, 16, "ADDONS/colorpicker/cursor.png", 0, 0, 0, self.color.white, self.gui.focus)
  dxDrawImageSection(x+280, y+24+(256-math.floor(256*self.color.h)), 48, 16, 16, 0, 48, 16, "ADDONS/colorpicker/cursor.png", 0, 0, 0, self.color.huecurrent, self.gui.focus)
...
function colorPicker:mouseSnap()
  if self.gui.track and source == self.gui.track then
    if self.color.s < self.gui.snaptreshold or self.color.s > 1-self.gui.snaptreshold then self.color.s = math.round_picker(self.color.s) end
    if self.color.v < self.gui.snaptreshold or self.color.v > 1-self.gui.snaptreshold then self.color.v = math.round_picker(self.color.v) end
    self:updateColor()
  end
end
...
function colorPicker:updateColor()
  ...
  guiSetText(self.gui.edith, "H: "..tostring(math.round_picker(self.color.h*360)).."°")
  guiSetText(self.gui.edits, "S: "..tostring(math.round_picker(self.color.s*100)).."%")
  guiSetText(self.gui.editv, "V: "..tostring(math.round_picker(self.color.v*100)).."%")
