...
function flug_chat(thePlayer, cmd, text, ...)
    if (isPedInVehicle(thePlayer)) then
        ...
        if (veh and (isPlane(veh) or isHeli(veh)) and (getPedOccupiedVehicleSeat(thePlayer) == 0)) then
            if (text) then
...
function sendFlugChatMessage(name, message)
    for theKey, thePerson in ipairs(getElementsByType("player")) do
        if (isPedInVehicle(thePerson)) then
            ...
            if (veh and (isPlane(veh) or isHeli(veh)) and (getPedOccupiedVehicleSeat(thePerson) == 0)) then
