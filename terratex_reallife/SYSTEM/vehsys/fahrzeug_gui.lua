...
addEventHandler("onClientResourceStart", resourceRoot,
    function()
        ...
        addEventHandler(
            ...
            FahrzeufUE_Button[8] ,
            function()
                abschleppTruck_Aufladen_GUI(lastclicked)
            end,
            false
        )
        addEventHandler("onClientGUIClick", FahrzeufUE_Button[9], abschleppTruck_Abladen_GUI, false)
    end)
    ...
    if not isAnyGuiVisible() then
        if (clickedWorld) then
            if (getElementType(clickedWorld) == "vehicle") then
                ...
                if (dis < 8) then
                    triggerServerEvent("showLastDriver", getLocalPlayer(), clickedWorld);
