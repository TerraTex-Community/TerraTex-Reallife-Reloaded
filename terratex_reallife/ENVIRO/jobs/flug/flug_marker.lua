    ...
    if (vioGetElementData(source, "heliLic") == 1) and (vioGetElementData(source, "planeLic") == 1) then
        if (getPlayerMoney(source) >= auftrag[1]) then
            ...
            if (auftrag[7] == 2) then
                ...
                tagetName = fly_typ_2_punkte[auftrag[6]][1]
            else
                ...
                tagetName = fly_typ_1_punkte[auftrag[6]][1]
            end
            ...
            if (source == vioGetElementData(driver, "flys_marker")) then
                if (getVehicleRealSpeed(theHitElement) < 41) then
                    ...
                    if (getVehicleType(theHitElement) == "Helicopter") then
                        marker = createMarker(1391.3000488281, 1814.4000244141, 10.8203125, "cylinder", 4, 88, 191, 162, 255, driver)
                        blip = createBlip(1391.3000488281, 1814.4000244141, 10.8203125, 0, 4, 88, 191, 162, 255, 0, 999999.0, driver)
                    else
                        marker = createMarker(1389.046875, 1701.9697265625, 10.8203125, "cylinder", 4, 88, 191, 162, 255, driver)
                        blip = createBlip(1389.046875, 1701.9697265625, 10.8203125, 0, 4, 88, 191, 162, 255, 0, 999999.0, driver)
                    end
