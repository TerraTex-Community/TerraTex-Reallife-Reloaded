...
function createInteriorMarker()
    ...
    --drogenhaus
    mark = createMarker(2166.3935546875, -1671.7578125, 15.074158668518, "corona", 2.0)
    ...
    --polizeiknast
    mark = createMarker(228.830078125, 114.5498046875, 999.015625, "corona", 2.0)
    polizeiraus = mark
    ...
    --ammoausen
    mark = createMarker(2556.96875, 2065.314453125, 11.099519729614, "corona", 2.0)
    ...
    --- -<<<<Startower >>>>>-------
    -- unten
    mark = createMarker(1567.7978515625, -1333.490234375, 16.484375, "corona", 2.0)
    table.insert(interiorMarker, { mark, 1541.37890625, -1363.5126953125, 329.796875, 0, 0, 0 })

    --oben
    mark = createMarker(1548.705078125, -1363.7421875, 326.21829223633, "corona", 2.0)
    ...
    --- -<<<<BANK>>>----
    -- drausen
    mark = createMarker(1381.412109375, -1088.748046875, 27.449289321899, "corona", 2.0)
    table.insert(interiorMarker, { mark, 1542.21875, 1691.552734375, 8373.9833984375, 1, 0, 0 })

    --innen
    mark = createMarker(1542.052734375, 1694.662109375, 8373.9833984375, "corona", 2.0)
    ...
    --- <<<Alhambra>>>----
    -- drausen
    mark = createMarker(1836.7255859375, -1682.9140625, 13.335935592651, "corona", 2.0)
    table.insert(interiorMarker, { mark, 493.940234375, -21.833984375, 1000.6796875, 17, 0, 0 })

    --innen
    mark = createMarker(493.1728515625, -24.326171875, 1000.6796875, "corona", 2.0)
    ...
    --- <<<Green Bottels>>>----
    -- drausen
    mark = createMarker(2310.2158203125, -1643.6650390625, 14.827047348022, "corona", 2.0)
    table.insert(interiorMarker, { mark, 502.8154296875, -71.1513671875, 998.7578125, 11, 0, 0 })

    --innen
    mark = createMarker(501.888671875, -68.2734375, 998.7578125, "corona", 2.0)
    table.insert(interiorMarker, { mark, 2307.0439453125, -1646.88671875, 14.754017829895, 0, 0, 0 })
    setElementInterior(mark, 11)

    --- <<<Pig Pen>>>----
    -- drausen
    mark = createMarker(2421.4716796875, -1219.5703125, 25.540721893311, "corona", 2.0)
    table.insert(interiorMarker, { mark, 1205.51171875, -10.9794921875, 1000.921875, 2, 0, 0 })

    --innen
    mark = createMarker(1204.8232421875, -13.6611328125, 1000.921875, "corona", 2.0)
    ...
    -- kirche
    --drausen
    mark = createMarker(2233.33203125, -1333.3525390625, 23.981576919556, "corona", 2.0)
    table.insert(interiorMarker, { mark, 2534.4267578125, -1235.7543945312, 21163.900390625, 2, 0, 0 })

    --innen
    mark = createMarker(2534.1826171875, -1232.5815429688, 21163.890625, "corona", 2.0)
    ...
    --tombupot LS
    --drausen
    mark = createMarker(1631.8291015625, -1172.669921875, 24.079999923706, "corona", 2.0)
    table.insert(interiorMarker, { mark, 830.9775390625, 7.99609375, 1004.1796875, 3, 0, 0 })

    --innen
    mark = createMarker(834.34997558594, 7.3800001144409, 1004.1900024414, "corona", 2.0)
    ...
    --drausen
    mark = createMarker(2202.0600585938, 2124.6599121094, 10.819999694824, "corona", 1.0)
    table.insert(interiorMarker, { mark, -2158.6398925781, 640.89001464844, 1052.3800048828, 1, 0, 0 })

    --innen
    mark = createMarker(-2158.6398925781, 642.89001464844, 1052.3800048828, "corona", 1.0)
    table.insert(interiorMarker, { mark, 2201.9921875, 2127.953125, 10.8203125, 0, 0, 0 })
    setElementInterior(mark, 1)

    --jefferson hotel
    --drausen
    mark = createMarker(2193.1748046875, -1151.26953125, 33.524044036865, "corona", 1.0)
    table.insert(interiorMarker, { mark, 2193.40234375, -1146.146484375, 1033.796875, 15, 0, 0 })

    --innen
    mark = createMarker(2193.3193359375, -1147.6650390625, 1033.796875, "corona", 1.0)
    table.insert(interiorMarker, { mark, 2193.255859375, -1154.1826171875, 33.524044036865, 0, 0, 0 })
    setElementInterior(mark, 15)

    --drausen
    mark = createMarker(2233.1064453125, -1159.6708984375, 25.890625, "corona", 1.0)
    table.insert(interiorMarker, { mark, 2216.3818359375, -1150.5595703125, 1025.796875, 15, 0, 0 })

    --innen
    mark = createMarker(2214.384765625, -1150.525390625, 1025.796875, "corona", 1.0)
