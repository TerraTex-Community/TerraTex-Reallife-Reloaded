    ...
    if (isPedInVehicle(thePlayer)) then
        ...
        if (vioGetElementData(vehicle, "isBusVeh")) then
            ...
            if (getVehicleOccupant(vehicle)) then
                outputChatBox("Ein Gast hat signalisiert, dass er an der nächsten Haltestelle aussteigen möchte!", getVehicleOccupant(vehicle), 0, 255, 0)
                triggerClientEvent(getVehicleOccupant(vehicle), "playSignalHaltestelle", getVehicleOccupant(vehicle))
            end
        end
    end
end
