...
-- Setzen und einfärben des Textes
function OnlineBanking_GetColor(element, text)
    local money = 0
    if (text == "money") then
...
-- Setzen und einfärben des Textes
function OnlineBanking_CalculateStats()
...
-- frmMain Initialisieren
function OnlineBanking_Initialize()
    OnlineBanking = {}
...
-- Anzeige Wechseln
function OnlineBanking_ToggleShow()
    if (OnlineBanking == nil) then
        ...
        if (guiGetVisible(OnlineBanking.frmMain) or guiGetVisible(OnlineBanking.frmUeberweisung) or guiGetVisible(OnlineBanking.frmTAN)) then
            guiSetVisible(OnlineBanking.frmMain, false)
            guiSetVisible(OnlineBanking.frmUeberweisung, false)
            guiSetVisible(OnlineBanking.frmTAN, false)
...
function obShallOfflineUeberweisung_func(empfaenger, grund, betrag)
    askCreate("Offlineüberweisung", "Ja! (Kosten: 1 Gold)", "Nein!", "Der Spieler ist Offline. Soll eine Offlineüberweisung durchgeführt werden?",
        function ()
            triggerServerEvent("obStartOfflineTransfer", getLocalPlayer(), empfaenger, grund, betrag);
        end
    ,nil)
end
addEvent("obShallOfflineUeberweisung", true);
addEventHandler("obShallOfflineUeberweisung", getRootElement(), obShallOfflineUeberweisung_func)
