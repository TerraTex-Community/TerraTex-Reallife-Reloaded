		...
		if(toPlayerName)then
			...
			for theKey,theVehicle in ipairs(privVeh) do
				if(theVehicle[1]==toPlayerName)then
					...
					local newmodel=MySQL_GetVar("vehicles", "Model", "ID='"..id.."'")
					setElementModel(ele,newmodel)
					local colorstr=MySQL_GetString("vehicles", "Colors", "ID='"..id.."'")	
					...
					local tuning=getStringComponents(MySQL_GetString("vehicles", "Tuning", "ID='"..id.."'")	)
