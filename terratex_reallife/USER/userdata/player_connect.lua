...
function LoginPlayerData(nickname, pw)
    ...
    if (passdb == pass) and not (vioGetElementData(source, "isLoggedInNow")) then
        ...
        if not (MySql.helper.existSync("user_gold", { Nickname = getPlayerName(source) })) then
            MySql.helper.insertSync("user_gold", { Nickname = nickname });
        end
        ...
        if (tonumber(userdataData["afktime"]) > 0) then
            outputChatBox("Aufgrund deiner Verstösse gegen das AFK System, wird deine Spielzeit weitere " .. tonumber(userdataData["afktime"]) .. " Minuten nicht gezählt");
        end
        ...
        vioSetElementData(source, "VehicleSpawn", tonumber(userdataData["PremiumSpawn"]))
        ...
        vioSetElementData(source, "industrialFishingLic", tonumber(lizensenData["industrialFishingLic"]))
