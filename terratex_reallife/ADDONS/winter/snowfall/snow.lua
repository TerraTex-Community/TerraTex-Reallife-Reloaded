        ...
        for i, flake in pairs(snowflakes) do
            if flake then
                ...
                if flake.z < (-box_height) then
                    ...
                    local gx, gy = 2, 2
                    if flake.x <= (box_width_doubled * 0.33) - box_width then
                        gx = 1
                    elseif flake.x >= (box_width_doubled * 0.66) - box_width then
                        gx = 3
                    end

                    if flake.y <= (box_depth_doubled * 0.33) - box_depth then
                        gy = 1
                    elseif flake.y >= (box_depth_doubled * 0.66) - box_depth then
                        gy = 3
                    end
                    ...
                    if ground[gx][gy] and (flake.z + lz) > ground[gx][gy] then
                        ...
                        outputDebugString(flake.x + lx + jitter_x)
                        outputDebugString(jitter_x, flake.y + ly + jitter_y)
                        outputDebugString( flake.z + lz)
