        ...
        if (isPedInVehicle(getLocalPlayer())) then
            ...
            if (veh) then
                if (getElementData(veh, "Beamtenvehicle")) then
                    ...
                    local textstring = "Geschwindigkeitsmesstation:\nFahrzeugkennzeichen:-\ngemessenes Tempo:-"
                    if isElement(nearestCar) then
...
function checkMyClearBlitzerLine()
    outputChatBox("recieved trigger")
    if (source == getLocalPlayer()) then
        outputChatBox("I hit it")
        if (isPedInVehicle(getLocalPlayer())) then
            outputChatBox("I am in the car")

            local bx, by, bz = getElementPosition(getElementData(source, "object"))
            outputChatBox(bx .. " ".. by .. " ".. bz)
            local px, py, pz = getElementPosition(getLocalPlayer())
...
function getTheNeartestCar_func()
    ...
    local nearestDistance = 70
    nearestCar = false
    for index, nearbyPlayer in ipairs(nearbyPlayers) do
        local cx, cy, cz = getElementPosition(nearbyPlayer)
