...
seperationSpace = 100 -- the space between top/bottom screen and scoreboard top/bottom in pixels
        ...
        if (currentTick - scoreboardTicks.lastUpdate > scoreboardTicks.updateInterval and (scoreboardToggled or scoreboardForced)) or forceScoreboardUpdate then
            ...
            if (showTeams or (serverInfo.forceshowteams and not serverInfo.forcehideteams)) and not serverInfo.forcehideteams then
                ...
                for key, team in ipairs( teams ) do
                    local mass = table.getn(getPlayersInTeam ( team ))
                    if(not(getElementData(team,"hidden")) and mass>0)then
            ...
            for key, player in ipairs( players ) do
                ...
                if(getPlayerTeam(player))then
                    showHidden = getElementData(getPlayerTeam(player),"hidden")
                end

                if not getPlayerTeam( player ) or showHidden or not (showTeams or (serverInfo.forceshowteams and not serverInfo.forcehideteams)) or serverInfo.forcehideteams then

                    newSorttable[sortTableIndex] = {}
                    for key, column in ipairs( scoreboardColumns ) do
                        local content
                        if column.name == "name" then
                            local playerName = getPlayerName( player )
                        ...
                        newSorttable[sortTableIndex][column.name] = content
                        newSorttable[sortTableIndex]["__SCOREBOARDELEMENT__"] = player

                    end
                    sortTableIndex = sortTableIndex + 1
                end
            end
            if sortBy.what ~= "__NONE__" then table.sort( newSorttable, scoreboardSortFunction ) end
            for key, value in ipairs( newSorttable ) do
                scoreboardContent[index] = value
                index = index + 1
            end
        ...
        if scoreboardDrawn then
            ...
            for key, column in ipairs( scoreboardColumns ) do
                ...
                if sortBy.what == column.name then
                    ...
                    dxDrawText( column.friendlyName or "-", topX+x+s(1+9), y+s(1), 	topX+x+s(1+column.width), 	y+s(1)+dxGetFontHeight( fontscale(columnFont, scoreboardScale), columnFont ), tocolor( 0, 0, 0, a ), fontscale(columnFont, s(1)), columnFont, column.align or "left", "top", true, false, drawOverGUI )
                    dxDrawText( column.friendlyName or "-", topX+x+s(9), 	y, 	topX+x+s(column.width), 	y+dxGetFontHeight( fontscale(columnFont, scoreboardScale), columnFont ), cHeader, fontscale(columnFont, s(1)), columnFont, column.align or "left", "top", true, false, drawOverGUI )
                    ...
                    local _, _, _, a = fromcolor( cHeader )
                    dxDrawText( column.friendlyName or "-", topX+x+s(1), 	y+s(1), 	topX+x+s(1+column.width), 	y+s(1)+dxGetFontHeight( fontscale(columnFont, scoreboardScale), columnFont ), tocolor( 0, 0, 0, a ), fontscale(columnFont, s(1)), columnFont, column.align or "left", "top", true, false, drawOverGUI )
                    dxDrawText( column.friendlyName or "-", topX+x, 	y, 	topX+x+s(column.width), 	y+dxGetFontHeight( fontscale(columnFont, scoreboardScale), columnFont ), cHeader, fontscale(columnFont, s(1)), columnFont, column.align or "left", "top", true, false, drawOverGUI )
                end
                    ...
                    for key, column in ipairs( scoreboardColumns ) do
                        ...
                        if content then
                            if serverInfo.allowcolorcodes and type( content ) == "table" and column.name == "name" then
                                ...
                                for k, v in ipairs( colorCodes ) do
                                    ...
                                    if firstCodePos ~= 1 and k == 1 then
                                        ...
                                        dxDrawText( partOfName, xPos+s(1), 	y+s(1), 	topX+x+s(1+column.width), 	y+s(11)+dxGetFontHeight( fontscale(teamHeaderFont, scoreboardScale), teamHeaderFont ), 	tocolor( 0, 0, 0, a or 255 ), fontscale(teamHeaderFont, s(1)), teamHeaderFont, column.align or "left", "top", true, false, drawOverGUI )
                                        dxDrawText( partOfName, xPos, 			y, 			topX+x+s(column.width), 	y+dxGetFontHeight( fontscale(teamHeaderFont, scoreboardScale), teamHeaderFont ), 	tocolor( r or 255, g or 255, b or 255, a or 255 ), fontscale(teamHeaderFont, s(1)), teamHeaderFont, column.align or "left", "top", true, false, drawOverGUI )
                            ...
                            elseif type( content ) == "table" and column.name ~= "name" then
                                ...
                                dxDrawText( content, topX+theX+s(1), 	y+s(1), 	topX+x+s(1+column.width), 	y+s(11)+dxGetFontHeight( fontscale(teamHeaderFont, scoreboardScale), teamHeaderFont ), 	tocolor( 0, 0, 0, a or 255 ), fontscale(teamHeaderFont, s(1)), teamHeaderFont, column.align or "left", "top", true, false, drawOverGUI )
                                dxDrawText( content, topX+theX, 		y, 		topX+x+s(column.width), 	y+dxGetFontHeight( fontscale(teamHeaderFont, scoreboardScale), teamHeaderFont ), 			tocolor( r or 255, g or 255, b or 255, a or 255 ), fontscale(teamHeaderFont, s(1)), teamHeaderFont, column.align or "left", "top", true, false, drawOverGUI )
                            end
                        end
                        x = x + s(column.width + 10)
                    end
                    ...
                    for key, column in ipairs( scoreboardColumns ) do
                        ...
                        if content then
                            if serverInfo.allowcolorcodes and type( content ) == "table" and column.name == "name" then
                                ...
                                for k, v in ipairs( colorCodes ) do
                                    ...
                                    if firstCodePos ~= 1 and k == 1 then
                                        ...
                                        local partOfName = string.sub( playerName, firstPos, secondPos )
                                        local textLength = dxGetTextWidth( partOfName, fontscale(contentFont, s(1)), contentFont )
                                        dxDrawText( partOfName, xPos+s(1), 	y+s(1), topX+x+s(1+column.width), 	y+s(11)+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), 	tocolor( 0, 0, 0, a or 255 ), fontscale(contentFont, s(1)), contentFont, column.align or "left", "top", true, false, drawOverGUI )
                                        dxDrawText( partOfName, xPos, 		y, 		topX+x+s(column.width), 	y+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), 			tocolor( r or 255, g or 255, b or 255, a or 255 ), fontscale(contentFont, s(1)), contentFont, column.align or "left", "top", true, false, drawOverGUI )
                                        xPos = xPos + textLength
                                    end
                                    if useColors then
                                        r, g, b = v[1][1], v[1][2], v[1][3]
                            ...
                            elseif type( content ) == "table" and column.name ~= "name" then
                                ...
                                dxDrawText( content, topX+theX+s(1), 	y+s(1), topX+x+s(1+column.width), 	y+s(11)+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), 	tocolor( 0, 0, 0, a or 255 ), fontscale(contentFont, s(1)), contentFont, column.align or "left", "top", true, false, drawOverGUI )
                                dxDrawText( content, topX+theX, 		y, 		topX+x+s(column.width), 	y+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), 			tocolor( r or 255, g or 255, b or 255, a or 255 ), fontscale(contentFont, s(1)), contentFont, column.align or "left", "top", true, false, drawOverGUI )
                            end
                        end
                        x = x + s(column.width + 10)
                    end
...
-- FUNCTIONS
-- addColumn
function scoreboardAddColumn( name, width, friendlyName, priority, textFunction, fromResource, align )
    if type( name ) == "string" then
        ...
        if not (priority > MAX_PRIRORITY_SLOT or priority < 1) then
            ...
            table.insert( scoreboardColumns, { ["name"] = name, ["width"] = width, ["friendlyName"] = friendlyName, ["priority"] = priority, ["textFunction"] = textFunction, ["align"] = align } )
...
addEventHandler( "doScoreboardAddColumn", getResourceRootElement(),
    function ( name, width, friendlyName, priority, align, fromResource )
        scoreboardAddColumn( name, width, friendlyName, priority, nil, fromResource ,align)
    end
)
...
function scoreboardResetColumns( fromServer )
    ...
    if not fromServer then
        scoreboardAddColumn( "name", 350, "Name" )
        scoreboardAddColumn( "ping", 40, "Ping" , nil, nil, nil, "right" )
    end
    return true
end
