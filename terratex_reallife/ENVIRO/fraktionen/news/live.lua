...
function accept_live(thePlayer)
    if (liveLeader) then
        local px, py, pz = getElementPosition(thePlayer)
    ...
    vioSetElementData(thePlayer, "isInviteToNewsLive", false)
end
registerAcceptHandler("live", accept_live, {
    requestedDataValues = {"isInviteToNewsLive"}
});
