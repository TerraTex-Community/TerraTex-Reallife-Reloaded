...
function tuning_einbauen()
    ...
    if (row ~= -1 and colo ~= -1) then
        ...
        if not (isinveh) then
            if (getPlayerMoney(getLocalPlayer()) < pricing + UpSlotPrice[slotnr]) and not isRacerBase then
...
function tuning_color_save()
    ...
    if not (getPlayerMoney(getLocalPlayer()) < 3400) or isRacerBase then
...
function showTuningGarage_func(color, paint, tuning, isRacer)
    isRacerBase = isRacer
    hitElement = source
