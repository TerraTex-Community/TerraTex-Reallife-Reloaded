...
scoreboardColumns = {
	{ ["name"] = "name", ["width"] = 350, ["friendlyName"] = "Name", ["priority"] = 1 },
	{ ["name"] = "ping", ["width"] = 40, ["friendlyName"] = "Ping", ["priority"] = MAX_PRIRORITY_SLOT, ["align"]="right" }
}
resourceColumns = {}
...
function scoreboardAddColumn( name, forElement, width, friendlyName, priority, align )
	if type( name ) == "string" then
		...
		if forElement == getRootElement() then
			if not (priority > MAX_PRIRORITY_SLOT or priority < 1) then
				...
				table.insert( scoreboardColumns, { ["name"] = name, ["width"] = width, ["friendlyName"] = friendlyName, ["priority"] = priority , ["align"] = align} )
				...
				return triggerClientEvent( getRootElement(), "doScoreboardAddColumn", getRootElement(), name, width, friendlyName, priority,align, sourceResource )
			end
		else
			return triggerClientEvent( forElement, "doScoreboardAddColumn", getRootElement(), name, width, friendlyName, priority,align, sourceResource )
		end
	end
	return false
end
...
function scoreboardResetColumns( forElement )
	...
	if forElement == getRootElement() then
		...
		if result then
			...
			scoreboardAddColumn( "ping", 40, "Ping" ,500, "right")
		end
		return result
	else
...
function onClientDXScoreboardResourceStart()
	for key, column in ipairs( scoreboardColumns ) do
		triggerClientEvent( client, "doScoreboardAddColumn", getRootElement(), column.name, column.width, column.friendlyName, column.priority, column.align )
	end
end
